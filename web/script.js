...
(async () => {
  try {
    ...
    document.getElementById("layoutToggle").addEventListener("click", () => {
      ...
      document.getElementById("layoutToggle").textContent = isHierarchical
        ? "Force Layout"
        : "Organize Look";
...
function renderGraph(data, hierarchical = false) {
  try {
    ...
    if (currentFilter === "direct") {
      nodes = nodes.filter(
        (node) => node.direct === true || node.main === true,
      );
    } else if (currentFilter === "indirect") {
      nodes = nodes.filter(
        (node) => node.direct === false && node.main !== true,
      );
    }
    ...
    simulation.on("tick", () => {
      ...
      rootG.attr(
        "transform",
        `translate(${Math.max(0, -minX + PADDING / 2)}, ${Math.max(0, -minY + PADDING / 2)})`,
      );
      if (hierarchical) {
...
function handleSearch(searchTerm) {
  ...
  if (!searchTerm) {
    ...
    currentLabels.attr("opacity", 1).attr("font-weight", "normal");

    currentLinks.attr("opacity", 0.6).attr("stroke", "#999");
  ...
  currentNodes
    .attr("opacity", (d) =>
      matchingNodeIds.has(d.id) ? 1 : connectedNodeIds.has(d.id) ? 0.5 : 0.1,
    )
  ...
  currentLabels
    ...
    .attr("font-weight", (d) =>
      matchingNodeIds.has(d.id) ? "bold" : "normal",
    );
